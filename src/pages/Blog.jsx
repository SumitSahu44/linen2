  ...
  return (
    <div className="py-24 bg-slate-50 min-h-screen font-sans">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* Header Section */}
        <div className="mb-16">
          <span className="text-blue-600 font-bold tracking-[0.3em] uppercase text-xs">Knowledge Hub</span>
          <h2 className="text-5xl font-black text-gray-900 mt-3 tracking-tighter">Insights & Articles</h2>
          <div className="w-24 h-2 bg-blue-900 mt-4 rounded-full"></div>
        </div>

        {/* Categories Bar */}
        <div className="flex flex-wrap gap-4 mb-16">
          {categories.map((cat, i) => (
            <button key={i} className={`px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${i === 0 ? 'bg-blue-900 text-white shadow-lg' : 'bg-white text-gray-500 hover:bg-blue-50 hover:text-blue-600 border border-gray-100'}`}>
              {cat}
            </button>
          ))}
        </div>
        ...
        <div className="relative bg-white rounded-[3.5rem] shadow-2xl border border-gray-100 overflow-hidden mb-16 group">
          <div className="grid grid-cols-1 lg:grid-cols-2">
            <div className="relative overflow-hidden">
              ...
              />
              <div className="absolute top-8 left-8 bg-blue-600 text-white px-6 py-2 rounded-xl font-black text-xs uppercase tracking-widest">Featured</div>
            </div>
            <div className="p-12 lg:p-20 flex flex-col justify-center">
              <span className="text-blue-600 font-bold text-sm uppercase tracking-widest mb-4">Manufacturing Excellence</span>
              ...
              <p className="text-gray-500 text-lg mb-10 font-medium">Learn how our Noida and Indore units achieved 100% efficiency using new German spinning technology...</p>
              <button className="flex items-center gap-4 text-blue-900 font-black uppercase tracking-[0.2em] text-sm group-hover:gap-6 transition-all">
          ...
          {blogPosts.map((post) => (
            <div key={post.id} className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden group hover:-translate-y-2 transition-all">
              <div className="h-64 overflow-hidden relative">
                ...
                <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-md px-4 py-1.5 rounded-lg text-blue-600 font-black text-[10px] uppercase tracking-widest">
                  {post.category}
                </div>
              </div>
              <div className="p-8">
                ...
                <h4 className="text-xl font-black text-gray-900 mb-4 tracking-tight group-hover:text-blue-600 transition-colors">{post.title}</h4>
                <p className="text-gray-500 text-sm font-medium mb-6 line-clamp-2">{post.excerpt}</p>
                <div className="pt-6 border-t border-gray-50">
                  <button className="text-blue-900 font-black text-[11px] uppercase tracking-[0.2em] flex items-center gap-2">
        ...
        <div className="mt-20 flex justify-center gap-3">
          {[1, 2, 3].map(n => (
            <button key={n} className={`w-12 h-12 rounded-2xl font-black transition-all ${n === 1 ? 'bg-blue-900 text-white shadow-lg' : 'bg-white text-gray-400 border border-gray-100 hover:bg-blue-50'}`}>
              {n}
            </button>
          ))}
        </div>
