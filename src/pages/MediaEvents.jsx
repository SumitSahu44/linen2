        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          <div className="max-w-2xl">
            <h1 className="text-6xl md:text-8xl font-black text-slate-900 uppercase leading-[0.85] tracking-tighter mb-6 italic">
              Media <br /> <span className="text-blue-600">& Events.</span>
            </h1>
            <p className="text-slate-500 font-medium leading-relaxed border-l-4 border-blue-600 pl-6">
          ...
          <div className="flex gap-4">
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center min-w-[100px]">
              <span className="text-2xl font-black text-blue-600 leading-none">25+</span>
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-2">Exhibitions</span>
            </div>
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center min-w-[100px]">
              <span className="text-2xl font-black text-blue-600 leading-none">10+</span>
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-12">
          {events.filter(e => e.featured).map((event, i) => (
            <div key={i} className="lg:col-span-3 relative h-[500px] rounded-[3rem] overflow-hidden group shadow-2xl">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/40 to-transparent flex flex-col justify-end p-10 md:p-16">
                <div className="flex items-center gap-3 text-blue-400 font-black text-xs uppercase tracking-[0.4em] mb-4">
                ...
                <div className="flex flex-wrap gap-8 text-white/80 font-bold uppercase text-[10px] tracking-[0.2em]">
                  ...
                  <span className="flex items-center gap-2"><MapPin size={14} className="text-blue-500" /> {event.location}</span>
                </div>
              </div>
            </div>
          ))}
          ...
          {events.filter(e => !e.featured).map((event, i) => (
            <div key={i} className="bg-white rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 group">
              ...
              <div className="p-8">
                <h3 className="text-xl font-black text-slate-900 uppercase leading-tight mb-4 group-hover:text-blue-600 transition tracking-tighter italic">
                  {event.title}
                </h3>
                ...
                <button className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-900 group-hover:text-blue-600 transition">
          ...
          {/* --- VIDEO PRESS CARD --- */}
          <div className="bg-blue-600 rounded-[2.5rem] p-10 text-white flex flex-col justify-between h-[400px] shadow-xl relative overflow-hidden group">
            <div className="absolute -top-10 -right-10 opacity-20 group-hover:scale-150 transition duration-1000">
               <PlayCircle size={200} />
            </div>
            <div className="relative z-10">
              <Newspaper className="text-blue-200 mb-6" size={32} />
              <h4 className="text-3xl font-black uppercase italic leading-none">Media & <br /> Press Kits.</h4>
              <p className="text-blue-100 text-sm mt-4 font-medium">Download our official high-resolution media kit for press and coverage.</p>
            </div>
            <button className="relative z-10 bg-white text-blue-900 w-full py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-slate-900 hover:text-white transition-all shadow-lg flex items-center justify-center gap-3">
