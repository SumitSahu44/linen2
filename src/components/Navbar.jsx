    ...
    <header className="relative w-full z-[1000]">
      ...
      <div className="hidden sm:flex bg-slate-900 text-white text-[10px] py-2 px-6 justify-between items-center uppercase tracking-[0.2em] font-bold">
        ...
        <div className="flex gap-6 items-center">
          <Link to="/trade-enquiry" className="hover:text-brand transition-colors">Trade Enquiry</Link>
      ...
      <nav className="bg-white border-b border-gray-100 sticky top-0 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex justify-between items-center">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            ...
            {/* Logo Image */}
            <img
              src="/Parekh.png"   // 👈 apna path check kar lena
              alt="Parekh Linen Logo"
          ...
          <div className="hidden lg:flex items-center space-x-8 font-bold text-gray-700 text-sm">
            {mainLinks.map((link) => (
              <Link key={link.name} to={link.path} className="hover:text-brand transition-colors py-2">
                {link.name}
              </Link>
            ))}
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full transition-all border ${isMenuOpen ? 'bg-brand text-white border-brand' : 'bg-slate-50 text-slate-900 border-slate-200 hover:border-brand'
                }`}
            >
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            <Link to="/contact" className="hidden sm:flex p-2.5 bg-slate-50 rounded-full hover:bg-brand/10 hover:text-brand transition-all text-slate-600">
              <MapPin size={18} />
            </Link>
          ...
          <div className="hidden lg:block absolute top-full left-0 w-full bg-white shadow-[0_20px_50px_rgba(0,0,0,0.1)] border-t border-gray-100 p-12 animate-in fade-in slide-in-from-top-4 duration-300">
            <div className="max-w-7xl mx-auto grid grid-cols-4 gap-12">
              ...
              {sections.map((section, idx) => (
                <div key={idx}>
                  <h4 className="flex items-center gap-2 text-brand font-black text-xs uppercase mb-6 tracking-widest">
                    {section.icon} {section.title}
                  </h4>
                  <ul className="space-y-4 text-gray-600 text-sm font-medium">
                    {section.links.map(link => (
                      <li key={link.name}>
                        <Link to={link.path} className="flex items-center gap-2 hover:text-brand hover:translate-x-1 transition-all">
              ...
              <div className="bg-slate-900 p-8 rounded-[2rem] text-white">
                <h4 className="text-brand font-black text-xs uppercase mb-4 tracking-widest">Connect with us</h4>
                ...
                <div className="flex gap-3">
                  <a href="#" className="w-9 h-9 bg-white/10 rounded-full flex items-center justify-center hover:bg-brand transition-all"><Facebook size={16} /></a>
                  <a href="#" className="w-9 h-9 bg-white/10 rounded-full flex items-center justify-center hover:bg-brand transition-all"><Twitter size={16} /></a>
        ...
        <div className={`lg:hidden fixed inset-0 z-[2000] bg-slate-900/50 backdrop-blur-sm transition-opacity duration-300 ${isMobileOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          <div className={`absolute right-0 top-0 h-full w-[80%] max-w-sm bg-white shadow-2xl transition-transform duration-300 ease-out p-6 overflow-y-auto ${isMobileOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            ...
            <div className="space-y-8">
              {sections.map((section, idx) => (
                <div key={idx}>
                  <h4 className="text-brand font-black text-[10px] uppercase tracking-[0.2em] mb-4">{section.title}</h4>
                  <div className="grid grid-cols-1 gap-3">
                    {section.links.map(link => (
                      <Link
                        key={link.name}
                        to={link.path}
                        className="text-slate-700 font-bold text-sm hover:text-brand py-1"
                      >
                        {link.name}
                      </Link>
                    ))}
            ...
            <div className="mt-12 pt-8 border-t">
              <Link to="/trade-enquiry" className="block w-full bg-brand text-white text-center py-4 rounded-xl font-black uppercase tracking-widest text-xs">
                Trade Enquiry
              </Link>
            </div>
          </div>
